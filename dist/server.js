...
const authenticateJWT = (req, res, next) => {
    ...
    if (token) {
        jsonwebtoken_1.default.verify(token, process.env.POSTGRES_JWT_SECRET, (err, user) => {
            if (err) {
...
app.post('/api/events', authenticateJWT, async (req, res) => {
    ...
    try {
        const result = await pool.query('INSERT INTO events (name, date, description, location, user_id, created_by) VALUES ($1, $2, $3, $4, $5, $6) RETURNING *', [name, date, description, location, req.user.id, req.user.fullname]);
...
app.get('/api/events/others', authenticateJWT, async (req, res) => {
    try {
        const result = await pool.query('SELECT events.*, users.fullname as created_by, ' +
...
app.get('/api/events/:id', authenticateJWT, async (req, res) => {
    const { id } = req.params;
    try {
        const eventResult = await pool.query('SELECT events.*, users.fullname as created_by FROM events JOIN users ON events.user_id = users.id WHERE events.id = $1', [id]);
        if (eventResult.rows.length === 0) {
            return res.status(404).json({ message: 'Event not found' });
        }
        const event = eventResult.rows[0];
        // Get attendees
        const attendeesResult = await pool.query('SELECT users.id, users.fullname as name FROM rsvps JOIN users ON rsvps.user_id = users.id WHERE rsvps.event_id = $1 AND rsvps.status = $2', [id, 'attending']);
...
// Join event endpoint
app.post('/api/events/join/:id', authenticateJWT, async (req, res) => {
    const { id } = req.params;
    ...
    try {
        ...
        const updatedEvent = await pool.query(`SELECT events.*, users.fullname as created_by, 
      (SELECT COUNT(*) FROM rsvps WHERE rsvps.event_id = events.id AND rsvps.status = 'attending') as attendee_count
      FROM events 
      JOIN users ON events.user_id = users.id 
...
app.post('/api/events/leave/:id', authenticateJWT, async (req, res) => {
    const { id } = req.params;
    try {
...
app.post('/api/events/:id/messages', authenticateJWT, async (req, res) => {
    const { id } = req.params;
    const { message } = req.body;
    try {
        const result = await pool.query('INSERT INTO messages (event_id, user_id, message) VALUES ($1, $2, $3) RETURNING *', [id, req.user.id, message]);
        res.status(201).json(result.rows[0]);
    }
    catch (error) {
...
app.get('/api/events/:id/messages', authenticateJWT, async (req, res) => {
    const { id } = req.params;
    try {
        const result = await pool.query('SELECT messages.*, users.fullname as sender FROM messages JOIN users ON messages.user_id = users.id WHERE messages.event_id = $1 ORDER BY messages.timestamp', [id]);
        res.json(result.rows);
    }
    catch (error) {
...
app.put('/api/events/:id', authenticateJWT, async (req, res) => {
    ...
    const { name, date, description, location } = req.body;
    try {
        const result = await pool.query('UPDATE events SET name = $1, date = $2, description = $3, location = $4 WHERE id = $5 AND user_id = $6 RETURNING *', [name, date, description, location, id, req.user.id]);
        if (result.rows.length === 0) {
